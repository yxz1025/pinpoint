...
public class ElasticQueryLogInfoDao implements ElasticLogDao{
    ...
    @Override
    public DataWithPaging<Map> scrollSearch(SearchRequestParams searchRequestParams) {
        ...
        //获取所有的values
        DataWithPaging<Map> dataSet = new DataWithPaging<>();
    ...
    private List<Map> processResultHits(SearchHit[] hits){
        List<Map> list = new ArrayList<>();
        Arrays.stream(hits).forEach(item -> processResultHitsItem(list, item));
        return list;
    }

    private void processResultHitsItem(List<Map> list, SearchHit item) {
